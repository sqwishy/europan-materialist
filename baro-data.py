...
def build_bundle(
    ...
    preitem_layers = (preitem_by_package[package.name] for package in load_order)
    ...
    return Bundle(
        ...
        entities=[
            BundleEntity(
                identifier=identifier,
                tags=item.tags,
                package=coerce_to_identifier(preitem.package.name)
                if (preitem := preitem_by_identifier[identifier])
                and not preitem.package.iscorepackage
                else None,
            )
            for identifier, item in index.items()
        ],
