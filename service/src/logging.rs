...
pub trait Logger {
    ...
    fn log<'a>(&mut self, lvl: Level, what: &str, wher: FileLineNo, kv: &[KeyValue<'a>]) {
        let ev = Event {
            lvl,
            when: Zoned::now(),
            wher,
...
fn clean_journal_key(dirty: &'_ str) -> Cow<'_, str> {
    ...
    let safe_start = dirty
        .trim_start_matches(|c: char| -> bool { !matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9') });
...
#[warn(deprecated)]
pub fn display<D: fmt::Display>(d: D) -> impl fmt::Display {
    d
}
...
impl<E: std::error::Error> fmt::Debug for LogError<E> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.0)?;
...
pub fn opt<'a, D: fmt::Display>(d: &'a Option<D>) -> impl fmt::Display + 'a {
    OptDisplay(d)
}

struct OptDisplay<'a, D>(&'a Option<D>);

impl<'a, D: fmt::Display> fmt::Display for OptDisplay<'a, D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.0 {
            Some(d) => write!(f, "{}", d),
            None => write!(f, "None"),
...
mod tests {
    ...
    fn test() {
        ...
        info!("hello world");
        warn!("yikes");
        crit!("oof"; "happy" => super::debug(&vec!["ducks"]), "owo" => 69);
    }
}
